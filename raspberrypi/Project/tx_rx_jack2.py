	...
	def send_data(self, node):
		if node == 1:
			...
			if self.pumpModeNode3 == 1:
				print("########################################### Mode AuTo ###########################################")
				if self.blynkUsageStateN1 == 1 and self.blynkUsageStateN2 == 1:
					print("########################################### Use Node1 and Node2 ###########################################")
					#AVERRAGE water Level
					averageWaterLevel = (self.waterLevelNode1+self.waterLevelNode2)/2
					if averageWaterLevel <= self.blynkWaterLevelStart:
						print("###################################### ON PUMP<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
						self.pumpStateNode3 = 1
					elif averageWaterLevel >= self.blynkWaterLevelStop:
						print("###################################### OFF PUMP<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
						self.pumpStateNode3 = 0
				elif self.blynkUsageStateN1 == 1:
					print("############################################### Use Node1 #################################################")
